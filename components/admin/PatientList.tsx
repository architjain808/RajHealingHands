...
/** Converts YYYY-MM-DD → DD-MMM-YYYY (e.g. 18-Mar-2026) */
...
/** Formats contact for display: if stored as +91XXXXXXXXXX, show "+91 XXXXXXXXXX" */
const formatContact = (contact: string, countryCode?: string): string => {
  const cc = countryCode || "";
  if (!cc) return contact;
  return `${cc} ${contact}`;
};
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-3 rounded-xl border-2 text-sm font-bold transition-all ${
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {paginated.map((patient, idx) => (
                    <tr
                      key={patient.id}
                      className={`border-b border-slate-50 hover:bg-teal-50/50 transition-colors ${
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                        {formatContact(patient.contact, patient.countryCode)}
                      </td>
                      <td className="px-4 py-3 text-slate-500">
                        {patient.age !== undefined ? patient.age : "—"}
                      </td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                        {formatDateDisplay(patient.date)}
                      </td>
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500 flex-wrap">
                  ...
                  {patient.age !== undefined && (
                    <span className="bg-violet-50 text-violet-600 px-1.5 py-0.5 rounded text-[10px] font-bold">
                      Age {patient.age}
                    </span>
                  )}
                  <span
                    className="truncate flex-1 text-slate-400"
                    title={patient.condition}
                  >
                    {patient.condition}
                  </span>
